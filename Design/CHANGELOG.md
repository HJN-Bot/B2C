...
## [2026-05-02] Sprint 2 — PRD v0.9 Product Direction Reset
...
下面是当前状态总结，可以直接给下一个 AI 接手。

**当前项目状态**
项目路径：`/Users/eva/Desktop/B2C/meaningfully`

用户目标：做一个面向中学生英语科普演讲练习的 AI 陪练 App。核心不是评分器，而是帮助用户开口、卡住时继续说、练完后得到下一次能直接用的 takeaway，并长期形成成长画像。

**今天主要完成的方向**
1. 实时练习页 `PracticeRoom`
   - 已经从分散的字幕/AI/音浪改成中间“AI is listening”主板块。
   - 底部有四栏 App Tab：`Start / Practice / Takeaway / My`。
   - 猫猫陪练形象已经接入。
   - 最新 alpha 多帧素材已复制到：
     - `public/assets/cat-coach/cat_listening_motion_alpha.png`
     - `public/assets/cat-coach/cat_thinking_motion_alpha.png`
     - `public/assets/cat-coach/cat_coaching_motion_alpha.png`
     - `public/assets/cat-coach/cat_excited_motion_alpha.png`
   - `PracticeRoom.tsx` 里已经改成使用 `cat-motion-coach-*` 类。
   - `src/index.css` 里已经加了 `.cat-motion-*` 的 4x2 sprite 动画 CSS。
   - 结束练习时，会把 session summary 写入：
     - `sessionStorage["meaningfully.lastSession"]`

2. Takeaway / AI Plan
   - 原来的 `SessionEnd.tsx` 文件读起来会卡住，疑似云盘/文件占位问题。
   - 已新建 `src/pages/TakeawayPage.tsx`，并在 `src/App.tsx` 中把 `/session-end` 路由改为 `TakeawayPage`。
   - `TakeawayPage` 设计为：
     - 优先读 route state
     - 如果没有 route state，则读 `sessionStorage["meaningfully.lastSession"]`
     - 优先读 `sessionStorage["meaningfully.geminiApiKey"]`
     - 如果没有缓存 key，再调用 Supabase Edge Function `get-gemini-api-key`
   - 已补本地 Edge Function 源码：
     - `supabase/functions/get-gemini-api-key/index.ts`
   - 注意：还没有确认远端 Supabase 是否已经部署这个 Edge Function，所以用户看到的 `Failed to send a request to the Edge Function` 可能仍然来自远端未部署/网络失败。

3. My Page
   - 已新建 `src/pages/MyPage.tsx`
   - 包含：
     - Practice history
     - Trying Point
     - Ability portrait
...
4. 文档
   ...
   - 记录了：
     ...
     - 四栏 App 结构
     - 猫猫陪练素材方向
     - Takeaway 应固定高度滚动，不能无限撑长

**当前已知问题 / 风险**
1. `npm run build` 刚才启动后没有及时返回，可能卡在读取旧 `SessionEnd.tsx` 或某些文件系统状态。
2. `src/pages/SessionEnd.tsx` 仍然存在，但已不再从 `App.tsx` 引用。这个文件读起来曾经卡住，不建议继续依赖它。
3. 如果构建仍卡，需要检查是否 Vite/TypeScript 仍扫描到 `SessionEnd.tsx`。因为 `tsconfig.app.json` include 了 `src`，未引用文件也可能被 TS 扫描。解决办法：
   - 把 `SessionEnd.tsx` 改名为 `SessionEnd.legacy.txt` 或移到 `Design/archive/`
   - 或删除它，但删除前最好备份
4. Supabase Edge Function 远端可能没有 `get-gemini-api-key`，需要部署：
   - `supabase/functions/get-gemini-api-key/index.ts`
5. `supabase/functions/.env` 不能随便暴露，但需要确认里面有 `GEMINI_API_KEY` 或 `GOOGLE_GENERATIVE_AI_API_KEY`。

**建议下一个 AI 的接手步骤**
1. 先跑：
   - `git status --short`
   - `npm run build`
2. 如果 build 卡住，优先处理 `src/pages/SessionEnd.tsx`：
   - 备份后改名或移出 `src/pages`
   - 确保 `App.tsx` 只引用 `TakeawayPage`
3. 再跑 build，确认通过。
4. 测试 Practice → End → Takeaway：
   - PracticeRoom 是否能写入 `meaningfully.lastSession`
   - TakeawayPage 是否能读到 transcript / highlightWords / ktvScore
   - Next Run Plan 是否从 AI 返回
   - Chatbox 预设按钮是否调用 Gemini
5. 如果 AI 仍失败：
   - 检查浏览器 console 的具体错误
   - 检查 Supabase Edge Function 是否已部署
   - 检查 function secrets 是否包含 Gemini key
   - 如果 PracticeRoom 已经成功拿到 key，确认 `sessionStorage["meaningfully.geminiApiKey"]` 是否存在
6. 猫猫动画后续可以继续优化：
   - 现在使用 4x2 alpha motion sheets
   - 如果要更精致，建议每个状态导出等尺寸、透明、无白边的 8 帧 sprite
   - 当前 CSS 在 `src/index.css` 里搜索 `cat-motion`

**当前重要文件**
- `src/pages/PracticeRoom.tsx`
- `src/pages/TakeawayPage.tsx`
- `src/pages/MyPage.tsx`
- `src/components/AppTabBar.tsx`
- `src/index.css`
- `src/App.tsx`
- `public/assets/cat-coach/`
- `supabase/functions/get-gemini-api-key/index.ts`
- `Design/PRD.md`
- `Design/CHANGELOG.md`

**一句话交接**
现在产品方向已经基本确定：四页 App 结构 + 实时猫猫 AI 陪练 + AI 生成 Next Run Plan + 可追问 Chatbox + My Page 长期画像。下一步主要是收口构建问题、清理旧 `SessionEnd.tsx` 文件、确认 Supabase Edge Function 部署，然后做真实端到端试用。